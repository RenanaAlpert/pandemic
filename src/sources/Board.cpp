...
int &CityData::getDiseases()
{
    return this->diseases;
}

void CityData::setResearches(bool b){
    this->is_researches = b;
}

// Print the name of the city, how many disease it has and if it research station or not.
ostream &pandemic::operator<<(ostream &os, const CityData &c)
{
    os << "| " << Board::enum_to_string(c.city) << "\t|\t" << c.diseases << "\t|\t" << c.is_researches << "\t|" << endl;
    return os;
}
...
// Loading black cities to the board.
...
// Loading yellow cities to the board.
...
// Loading red cities to the board.
void Board::redCities()
{
    ...
    board_map[City::Manila] = CityData(City::Manila, Color::Red, 0, false, {City::Taipei, City::SanFrancisco, City::HoChiMinhCity, City::Sydney, City::HongKong});
...
// Loading blue cities to the board.
...
//get number of disease ib the given city.
int &Board::operator[](pandemic::City city)
{
    return board_map.at(city).getDiseases();
}

// Return true if there is not city sick, else false.
bool Board::is_clean()
{
    map<pandemic::City, CityData>::iterator itr;
    for (itr = this->board_map.begin(); itr != this->board_map.end(); itr++)
    {
        if (itr->second.getDiseases() > 0)
        {
            return false;
        }
    }
...
// Remove all the research stations. To tests.
void Board::remove_stations()
{
    map<pandemic::City, CityData>::iterator itr;
    for (itr = this->board_map.begin(); itr != this->board_map.end(); itr++)
    {
        itr->second.setResearches(false);
    }
}

map<pandemic::City, CityData> &Board::get_board(){
    return this->board_map;
}

// Print tall the cities in the board.
ostream &pandemic::operator<<(ostream &os, const Board &b)
{
    os << "----------------------------------------------------------------------------------" << endl;
    os << "|" << "\tcity\t" << "|" << "\tdisease level\t" << "|" << "\tis_research_station\t" << "|" << endl;
    os << "----------------------------------------------------------------------------------" << endl;
    map<pandemic::City, CityData>::iterator itr;
    for (auto itr = b.board_map.begin(); itr != b.board_map.end(); itr++)
    {
        os << itr->second << endl;
    }
    os << "----------------------------------------------------------------------------------" << endl;
